...
Функция ИмяЛога() Экспорт
	Возврат "oscript.lib.gitsync.plugins.edt";
КонецФункции
...
Процедура ПриРегистрацииКомандыПриложения(ИмяКоманды, КлассРеализации) Экспорт
	...
	КлассРеализации.Опция("P project-name", "", "[*edt] Имя проекта")
	.ТСтрока()
	.ВОкружении("GITSYNC_PROJECT_NAME");
	
	КлассРеализации.Опция("W workspace-location", "", "[*edt] расположение рабочей области")
	.ТСтрока()
	...
	КлассРеализации.Опция(
		"B base-project-name",
		"",
		"[*edt] имя базового проекта в рабочей области (для расширений))")
	.ТСтрока()
	...
	КлассРеализации.Опция(
		"M module",
		"",
		"[*edt] имя установленного модуля edt")
	.ТСтрока()
...
Процедура ПередСборкойCFФайла(
	...
	Лог.Отладка("Каталог для єкспорта из edt в  xml:%1", КаталогРабочейКопииВрем);
	...
	Параметры.Добавить(СтрШаблон("--configuration-files ""%1""", ВременноеРабочееПространство));
	Параметры.Добавить(СтрШаблон("--workspace-location ""%1""", РабочееПространство));
	Параметры.Добавить(СтрШаблон("--project ""%1""", КаталогРабочейКопииВрем));
	...
	Команда.УстановитьСтрокуЗапуска(СтрШаблон("ring %1 workspace export ", ИмяМодуля));
	Лог.Отладка("Строка запуска:"+СтрШаблон("ring %1 workspace export ", ИмяМодуля)+ СтрСоединить(параметры," "));
	Команда.УстановитьКодировкуВывода(КодировкаТекста.ANSI);
	Команда.ДобавитьЛогВыводаКоманды("oscript.lib.gitsync.plugins.edt");
	Команда.ДобавитьПараметры(Параметры);
	Команда.УстановитьИсполнениеЧерезКомандыСистемы(Истина);
	Команда.ПоказыватьВыводНемедленно(Ложь);
	Команда.УстановитьПравильныйКодВозврата(0);
	КодВозврата = Команда.Исполнить();
	Сообщить(СтрШаблон("ring %1 workspace export", ИмяМодуля)+ СтрСоединить(параметры," "));
...
Процедура Инициализация()
	...
	ИмяФайлаИзменений = "dumplist.txt"; // файл используется в 2х плагинах - increment и edt
